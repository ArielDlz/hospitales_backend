export interface RecordatorioPruebasEmailParams {
  nombre: string;
  loginUrl: string;
}

export interface RecordatorioPruebasEmailContent {
  subject: string;
  html: string;
  text: string;
}
...
const WHATSAPP = '+525527592438';

export function buildRecordatorioPruebasEmail(
  params: RecordatorioPruebasEmailParams,
): RecordatorioPruebasEmailContent {
  ...
  const text = [
    ...
    `Hola, ${nombre}`,
    '',
    'Notamos que tienes evaluaciones que no has realizado o están incompletas, te invitamos a que las realices lo más pronto que puedas para que tu proceso pueda seguir.',
    '',
    `Si tienes dudas o presentaste algún problema por favor comunícate vía whatsApp al ${WHATSAPP}.`,
    '',
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
          ...
          <tr>
            <td style="padding:32px;">
              ...
              <p style="margin:0 0 16px;font-size:16px;line-height:1.5;">
                Si tienes dudas o presentaste algún problema por favor comunícate vía whatsApp al ${escapeHtml(WHATSAPP)}.
              </p>
